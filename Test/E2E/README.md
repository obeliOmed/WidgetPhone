# E2E tests — WidgetPhone

Playwright suite for `WidgetPhone`. Config in `../../playwright.config.ts`. Playwright is resolved from the `/GitHub` workspace root (no local `@playwright/test` devDep).
...
- Plugin `WidgetPhone` activated in FS.
